...
- fix: `gfs commit` failed whenever a different `cp` preceded the system one on `PATH`. Every storage backend shells out with flags written for a specific coreutils — `-cRp` on macOS, `--reflink=auto -a` on Linux — and Homebrew's coreutils installs a GNU `cp` many developers put ahead of `/usr/bin`, so every snapshot died with `cp: invalid option -- 'c'` and an error naming no cause. All three backends now resolve `cp` to `/bin` or `/usr/bin`, falling back to `PATH` only when neither has it, so a distribution that ships no `/bin/cp` (NixOS) still works. The behaviour of a commit no longer depends on the user's shell configuration
