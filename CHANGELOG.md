...
- fix: `gfs commit` failed on macOS whenever a GNU `cp` preceded `/usr/bin` on `PATH` — Homebrew's coreutils installs one, and `-c` is a BSD flag, so every snapshot died with "cp: invalid option -- 'c'" and an error that named no cause. The APFS adapter now resolves `/bin/cp` by absolute path rather than through `PATH`; a commit no longer depends on the user's shell configuration
