...
- fix: `gfs status` and the MCP `status` tool disagreed about the HEAD commit — the MCP tool injected one into its own payload while the CLI reported none in any output mode, and the two skill files documented the same command two different ways. It now lives on the shared `StatusResponse`, so both surfaces report it from one place and neither can drift
