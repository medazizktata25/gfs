...
- fix: `gfs checkout` could hand you content the target never contained. A branch's working copy is keyed by branch NAME and outlived `gfs branch -d`, so a later branch reusing the name inherited the deleted branch's workspace — uncommitted rows included — while checkout reported success; a commit taken from there records a diff that never happened. A detached workspace is named by a commit hash, an immutable name for one content state, and was likewise reused after being mutated. Deleting a branch now removes its workspace, and returning to a detached commit rebuilds it from the snapshot. Reusing a branch's workspace to preserve uncommitted work is unchanged — that is what it is for
...
- fix: `gfs commit` on a detached HEAD wrote the commit object and updated nothing — no ref, not even HEAD — so the commit was unreachable the moment it was created, reported as a success whose branch field was the full 64-character hash. There is no reflog to recover it from. HEAD now advances to the new commit, as in git, and the CLI says the commit is on no branch and how to keep it
