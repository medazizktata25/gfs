...
impl<R: DatabaseProviderRegistry> CommitRepoUseCase<R> {
    pub fn new(
        ...
        Self { repository, compute, storage, registry }
    }
    ...
    pub async fn run(
        ...
        if let (Some(runtime), Some(env)) = (&runtime_config, &environment) {
            ...
            self.compute.prepare_for_snapshot(&instance_id, &commands).await?;
        ...
        let snapshot_dest = self.repository.ensure_snapshot_path(&path, &snapshot_hash).await?;

        // COW-copy the workspace data dir into the snapshot destination folder.
        self.storage
            .snapshot(
                &volume_id,
                SnapshotOptions { label: Some(snapshot_dest.to_string_lossy().into_owned()) },
            )
            .await?;
...
mod tests {
    ...
    use crate::ports::database_provider::{
        ...
        ProviderError, Result as RegistryResult, SupportedFeature, SIGTERM,
    };
    use crate::ports::repository::{LogOptions, RemoteOptions, Repository};
    use crate::ports::storage::{
        CloneOptions, MountStatus, Quota, Snapshot, SnapshotId, SnapshotOptions,
    ...
    impl Repository for MockRepository {
        async fn init(&self, _: &std::path::Path, _: Option<String>) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn get_workspace_data_dir_for_head(&self, _: &std::path::Path) -> crate::ports::repository::Result<PathBuf> {
            Ok(PathBuf::from("/workspace/data"))
        }
        async fn update_environment_config(&self, _: &std::path::Path, _: crate::model::config::EnvironmentConfig) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn update_runtime_config(&self, _: &std::path::Path, _: RuntimeConfig) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn clone_repo(&self, _: &str, _: &std::path::Path) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn commit(&self, _: &std::path::Path, new_commit: NewCommit) -> crate::ports::repository::Result<String> {
            *self.committed.lock().unwrap() = Some(new_commit);
            Ok(self.commit_hash.clone())
        }
        async fn checkout(&self, _: &std::path::Path, _: &str) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn create_branch(&self, _: &std::path::Path, _: &str, _: &str) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn log(&self, _: &std::path::Path, _: LogOptions) -> crate::ports::repository::Result<Vec<CommitWithRefs>> { Ok(vec![]) }
        async fn rev_parse(&self, _: &std::path::Path, _: &str) -> crate::ports::repository::Result<String> { Ok(String::new()) }
        async fn push(&self, _: &std::path::Path, _: RemoteOptions) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn pull(&self, _: &std::path::Path, _: RemoteOptions) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn fetch(&self, _: &std::path::Path, _: RemoteOptions) -> crate::ports::repository::Result<()> { Ok(()) }
        async fn get_current_branch(&self, _: &std::path::Path) -> crate::ports::repository::Result<String> { Ok("main".to_string()) }
        async fn get_current_commit_id(&self, _: &std::path::Path) -> crate::ports::repository::Result<String> { Ok(self.current_commit.clone()) }
        async fn get_runtime_config(&self, _: &std::path::Path) -> crate::ports::repository::Result<Option<RuntimeConfig>> { Ok(self.runtime_config.clone()) }
        async fn get_mount_point(&self, _: &std::path::Path) -> crate::ports::repository::Result<Option<String>> { Ok(self.mount_point.clone()) }
        async fn get_environment_config(&self, _: &std::path::Path) -> crate::ports::repository::Result<Option<EnvironmentConfig>> { Ok(self.environment.clone()) }
        async fn get_user_config(&self, _: &std::path::Path) -> crate::ports::repository::Result<Option<UserConfig>> { Ok(self.user_config.clone()) }
        async fn ensure_snapshot_path(&self, _: &std::path::Path, hash: &str) -> crate::ports::repository::Result<PathBuf> {
            // Return a predictable temp-dir path for testing.
            Ok(PathBuf::from(format!("/tmp/snapshots/{}/{}", &hash[..2], &hash[2..])))
        }
        async fn get_active_workspace_data_dir(&self, _: &std::path::Path) -> crate::ports::repository::Result<PathBuf> {
            Ok(PathBuf::from("/workspace/data"))
        }
    }
    ...
    impl crate::ports::compute::Compute for MockCompute {
        async fn provision(&self, _: &ComputeDefinition) -> crate::ports::compute::Result<InstanceId> {
            Ok(InstanceId("mock".into()))
        }
        async fn start(&self, id: &InstanceId, _: StartOptions) -> crate::ports::compute::Result<InstanceStatus> {
            Ok(InstanceStatus { id: id.clone(), state: InstanceState::Running, pid: None, started_at: None, exit_code: None })
        }
        async fn stop(&self, id: &InstanceId) -> crate::ports::compute::Result<InstanceStatus> {
            Ok(InstanceStatus { id: id.clone(), state: InstanceState::Stopped, pid: None, started_at: None, exit_code: None })
        }
        ...
        async fn status(&self, id: &InstanceId) -> crate::ports::compute::Result<InstanceStatus> {
            Ok(InstanceStatus { id: id.clone(), state: self.state.clone(), pid: None, started_at: None, exit_code: None })
        }
        ...
        async fn logs(&self, _: &InstanceId, _: LogsOptions) -> crate::ports::compute::Result<Vec<LogEntry>> { Ok(vec![]) }
        async fn pause(&self, id: &InstanceId) -> crate::ports::compute::Result<InstanceStatus> {
            *self.paused.lock().unwrap() = true;
            Ok(InstanceStatus { id: id.clone(), state: InstanceState::Paused, pid: None, started_at: None, exit_code: None })
        }
    ...
    impl crate::ports::storage::StoragePort for MockStorage {
        async fn mount(&self, _: &VolumeId, _: &std::path::Path) -> crate::ports::storage::Result<()> { Ok(()) }
        async fn unmount(&self, _: &VolumeId) -> crate::ports::storage::Result<()> { Ok(()) }
        async fn snapshot(&self, id: &VolumeId, options: SnapshotOptions) -> crate::ports::storage::Result<Snapshot> {
        ...
        async fn clone(&self, _: &VolumeId, target: VolumeId, _: CloneOptions) -> crate::ports::storage::Result<VolumeStatus> {
            Ok(VolumeStatus { id: target, mount_point: None, status: MountStatus::Unmounted, size_bytes: 0, used_bytes: 0 })
        }
        async fn status(&self, id: &VolumeId) -> crate::ports::storage::Result<VolumeStatus> {
            Ok(VolumeStatus { id: id.clone(), mount_point: None, status: MountStatus::Unmounted, size_bytes: 0, used_bytes: 0 })
        }
        async fn quota(&self, id: &VolumeId) -> crate::ports::storage::Result<Quota> {
            Ok(Quota { volume_id: id.clone(), limit_bytes: 0, used_bytes: 0, free_bytes: 0 })
        }
    }
    ...
    impl DatabaseProvider for MockProvider {
        ...
        fn default_args(&self) -> Vec<DatabaseProviderArg> { vec![] }
        fn default_signal(&self) -> u32 { SIGTERM }
        fn connection_string(&self, _: &ConnectionParams) -> std::result::Result<String, ProviderError> {
    ...
    impl DatabaseProviderRegistry for MockRegistry {
        fn register(&self, _: Arc<dyn DatabaseProvider>) -> RegistryResult<()> { Ok(()) }
        ...
        fn list(&self) -> Vec<String> { vec!["mock-db".into()] }
        fn unregister(&self, _: &str) -> Option<Arc<dyn DatabaseProvider>> { None }
    }
    ...
    async fn empty_message_returns_error() {
        ...
        let err = uc.run(PathBuf::from("/repo"), "".into(), None, None, None, None).await;
    ...
    async fn commit_without_database_skips_compute() {
        ...
        let hash = uc
            .run(existing_repo_path(), "initial commit".into(), None, None, None, None)
            .await
    ...
    async fn commit_with_running_database_pauses_and_unpauses() {
        ...
        let hash = uc
            .run(existing_repo_path(), "second commit".into(), None, None, None, None)
            .await
            .expect("commit should succeed");

        assert_eq!(hash, "def456");
        assert!(*compute.prepared.lock().unwrap(), "prepare_for_snapshot should have been called");
        assert!(*compute.paused.lock().unwrap(), "pause should have been called");
        assert!(*compute.unpaused.lock().unwrap(), "unpause should have been called");
        assert_eq!(storage.last_volume.lock().unwrap().as_deref(), Some("/vol/main"));
        ...
        assert!(label.contains("/tmp/snapshots/"), "expected snapshot inside snapshots dir");
    }
    ...
    async fn commit_snapshot_hash_is_64_char_hex_independent_of_storage_id() {
        ...
        uc.run(existing_repo_path(), "snap test".into(), None, None, None, None)
            .await
            .expect("commit should succeed");
        ...
        assert!(label.contains("/tmp/snapshots/"), "expected snapshot dest under /tmp/snapshots, got: {label}");
        ...
        assert!(reconstructed_hash.chars().all(|c| c.is_ascii_hexdigit()), "hash should be hex");
    }
    ...
    async fn commit_uses_mount_point_as_volume_id() {
        ...
        uc.run(existing_repo_path(), "mp test".into(), None, None, None, None)
            .await
            .expect("commit should succeed");
    ...
    async fn commit_falls_back_to_workspace_data_dir_when_no_mount_point() {
        ...
        uc.run(existing_repo_path(), "fallback test".into(), None, None, None, None)
            .await
            .expect("commit should succeed");
